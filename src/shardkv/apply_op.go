...
func (kv *ShardKV) doGet(key string) (err Err, val string) {
...
func (kv *ShardKV) applyOp(msg raft.ApplyMsg, op Op) {
	...
	isRepeated := kv.duplicateFilter(shardId, op.ClientId, op.MsgId)
	...
	if kv.configReady(op.ConfigNum, op.Key) == OK {
		...
		case "Put":
			if !isRepeated {
				...
				kv.ids[shardId][op.ClientId] = op.MsgId
			}
		case "Append":
			if !isRepeated {
				_, v := kv.doGet(op.Key)
		...
		if ch, ok := kv.notifyCh[op.ReqId]; ok {
			nm := NotifyMsg{Err:OK}
			if op.Op == "Get" {
				nm.Err, nm.Value = kv.doGet(op.Key)
			}
			ch <- nm
		}
		kv.mu.Unlock()
...
func (kv *ShardKV) applyConfig(msg raft.ApplyMsg, config shardmaster.Config) {
	...
	data := make(map[int]MergeShardData)
	for _, shardId := range deleteShardIds {
		mergeShardData := MergeShardData{
			ConfigNum:  oldConfig.Num,
			ShardNum:   shardId,
			Data:       kv.data[shardId],
			MsgIndexes: kv.ids[shardId],
		}
		data[shardId] = mergeShardData
		kv.data[shardId] = make(map[string]string)
		kv.ids[shardId] = make(map[int64]int64)
	}
	kv.historyShards[oldConfig.Num] = data
...
func (kv *ShardKV) applyMergeShardData(msg raft.ApplyMsg, data MergeShardData) {
	...
	kv.ids[data.ShardNum] = make(map[int64]int64)
	for k, v := range data.Data {
		kv.data[data.ShardNum][k] = v
	}
	for k, v := range data.MsgIndexes {
		kv.ids[data.ShardNum][k] = v
	}
	delete(kv.waitShardIds, data.ShardNum)
	go kv.sendCleanShardData(kv.oldConfig, data.ShardNum)
}
